      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {cats.length} tags · adjust monthly limits below
          </p>
        </div>
          ...
          <div key={c.id} className="card-soft p-5 group">
            <div className="flex items-start gap-3">
              <div
                ...
                style={{
                  backgroundColor: (c.color || PALETTE[0]) + "22",
              ...
              <div className="flex-1 min-w-0 space-y-3">
                <Input
                  ...
                  className="font-medium border-transparent hover:border-border p-0 focus:p-2 focus:border-input transition-all h-auto"
                />
                <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {PALETTE.map((p) => (
                    <button
                      key={p}
                      onClick={() => update(c.id, { color: p })}
                      className={`h-5 w-5 rounded-full transition-transform ${(c.color || PALETTE[0]).toLowerCase() === p.toLowerCase() ? "ring-2 ring-offset-2 ring-primary scale-110" : "hover:scale-110"}`}
          ...
          <div className="flex flex-wrap gap-2">
            {PALETTE.map((p) => (
              <button
                ...
                type="button"
                onClick={() => setColor(p)}
                className={`h-7 w-7 rounded-full transition-transform ${color === p ? "ring-2 ring-offset-2 ring-primary" : "hover:scale-110"}`}
                style={{ backgroundColor: p }}
              />
            ))}
          </div>
